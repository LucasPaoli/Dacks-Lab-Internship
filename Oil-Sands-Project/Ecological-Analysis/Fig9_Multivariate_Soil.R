...
title("Layer effect : p-value = 0.938")
ordiplot(nmds,display = "sites")
ordihull(nmds,groups=as.character(env.matrix[,2]),draw="polygon",col="grey90",label=T,cex=1)
title("Treatment effect : p-value < 0.001")
ordiplot(nmds,display = "sites")
ordihull(nmds,groups=as.character(env.matrix[,3]),draw="polygon",col="grey90",label=T,cex=1)
title("Treatment*Layer effect : p-value < 0.001")
mtext("B - Exploratory Analysis of Soil Dataset : NMDS",outer=T,cex=1.5,padj=-1)
dev.off()
...
#PLOT
pdf(file = "Figure9A.pdf",width=6,height=6)
par(mfcol=c(1,1))
...
pdf(file = "Figure9B.pdf",width=6,height=6)
par(mfcol=c(1,1))
