...
pdf(file = "Figure4.pdf", width=17, height=6)
...
Barplot = barplot(
  ...
  assignment.matrix(Class_sub[,1:9],OTU_sub[,1:9]),
  assignment(t(Class.season)[,3])*June,
  assignment.matrix(Class_sub[,19:27],OTU_sub[,19:27]),
  assignment(t(Class.season)[,2])*September,
  assignment.matrix(Class_sub[,10:18],OTU_sub[,10:18])
  ),
  ylab="Number of OTUs",
  ylim=c(0,350),
...
legend("topright", # places a legend at the appropriate place 
       ...
       col=c("steelblue","grey40"),
       cex=1)
text(cex=0.8, x=Barplot, y = par("usr")[3]-5, legend_BML, xpd=TRUE, srt=45,adj=1)

lines(x=c(Barplot[3],Barplot[11]),y=c(100,100))
text(x=(Barplot[3]+Barplot[11])/2, y=100, "May", pos=3, cex=0.8)
lines(x=c(Barplot[13],Barplot[21]),y=c(100,100))
text(x=(Barplot[13]+Barplot[21])/2, y=100, "June", pos=3, cex=0.8)
lines(x=c(Barplot[23],Barplot[31]),y=c(100,100))
text(x=(Barplot[23]+Barplot[31])/2, y=100, "September", pos=3, cex=0.8)
...
Barplot = barplot(
  cbind(
    ...
    assignment.matrix(Class_soil[,1:6],OTU_soil[,1:6]),
    assignment(t(Class.reclamation)[,3])*Overburden,
    assignment.matrix(Class_soil[,13:18],OTU_soil[,13:18]),
    assignment(t(Class.reclamation)[,2])*Tailings,
    assignment.matrix(Class_soil[,7:12],OTU_soil[,7:12])
  ),
  ylab="Number of OTUs",
  col=c("steelblue","grey40"),
  main="B - Soil dataset")
...
text(cex=0.8, x=Barplot, y = par("usr")[3]-10, legend_soil, xpd=TRUE, srt=45,adj=1)

lines(x=c(Barplot[3],Barplot[8]),y=c(1000,1000))
text(x=(Barplot[3]+Barplot[8])/2, y=1000, "Undisturbed", pos=3, cex=0.8)
lines(x=c(Barplot[10],Barplot[15]),y=c(1000,1000))
text(x=(Barplot[10]+Barplot[15])/2, y=1000, "Overburden", pos=3, cex=0.8)
lines(x=c(Barplot[17],Barplot[22]),y=c(1000,1000))
text(x=(Barplot[17]+Barplot[22])/2, y=1000, "Tailings", pos=3, cex=0.8)

mtext("Number of OTUs and percentage of OTUs\nwith a resolved taxonomic assignment",outer=T,cex=1.5)
