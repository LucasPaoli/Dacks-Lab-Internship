...
pdf(file = "FigureS2.pdf", width=15, height=10)
par(mfrow=c(2,3),oma=c(0,4,4,0))
...
title("A - Accumulation curve for the whole lake over the three time points")

mtext(side=2,padj=-5,"BASE MINE LAKE")

rarecurve(t(Species_sub), step = 20, sample = raremax.bml, col = "steelblue", cex = 0.5,xlab="Sampling size (Number of sequences)")
title("B - Rarefaction curve and effect of rarefaction on each sample")
...
plot(sort(M[,1],decreasing=T),type="n",ylim=c(0,0.5),xlim=c(0,50),ylab="Relative abundance",
     main="C - Decreasing relative abundances of species per sample",
     xlab="Species index")
...
legend("topright", # places a legend at the appropriate place 
       ...
       cex=1) # gives the legend lines the correct color and width
...
title("D - Accumulation curve for the soil dataset")

mtext(side=2,padj=-5,"SOIL DATASET")

rarecurve(t(Species_soil), step = 20, sample = raremax.soil, col = "steelblue", cex = 0.6,xlab="Sampling size (Number of sequences)")
title("E - Rarefaction curve and effect of rarefaction on each sample")
...
plot(sort(M[,1],decreasing=T),type="n",ylim=c(0,0.15),xlim=c(0,350),ylab="Relative abundance",
     main="F - Decreasing relative abundances of species per sample",
     xlab="Species index")
for (i in 1:ncol(M)){lines(sort(M[,i],decreasing=T),col="black",lwd=2)}
for (i in 1:ncol(M1)){lines(sort(M1[,i],decreasing=T),col="steelblue",lwd=2)}
...
mtext("Rarefaction effect and accumulation curves",outer=T,cex=1.5)
