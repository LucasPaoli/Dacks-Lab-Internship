...
#
# Usearch : http://www.drive5.com/usearch/ (binary to download, no install needed)
...
PATHDATA="/Users/Lucas/Documents/ENS/BIO_M1_stage/OTU_scripts_writing"
OUTPUT="/Users/Lucas/Documents/ENS/BIO_M1_stage/OTU_scripts_writing/Output_uparse"
N_THREADS=4
DB_PR2="/Users/Lucas/Documents/ENS/BIO_M1_stage/OTU_clustering_LP/DB/gb203_pr2.fasta"
DB_SILVA="/Users/Lucas/Documents/ENS/BIO_M1_stage/OTU_clustering_LP/DB/SILVA_123_SSURef_Nr99_tax_silva.fasta"
SAMPLES="7C 7D 7E 7F 7G 7H 8A 8B 8C 8D 8E 8F 8G 8H 9A 9B 9C 9D"
...
do
sed -i .txt "s/@""${LOOP}""/@SMPL"${sample_number}":""${LOOP}""/g" "${PATHDATA}"/${i}*.fastq
...
sed -i .txt $'s/\t/;/g' "${OUTPUT}"/otu_bacteria_output.txt
...
sed -i .txt '/^$/d' "${OUTPUT}"/otu_bacteria.txt
...
sed -i .txt "/"$y$'\t/d' "${OUTPUT}"/otutab.txt
done
...
sed -i .txt $'s/\t/;/g' "${OUTPUT}"/otu_taxonomy.txt
cp "${OUTPUT}"/otu_taxonomy.txt "${OUTPUT}"/otu_taxonomy_save.txt
sed -i .txt $'s/\|Eukaryota\|/;Eukaryota\|/g' "${OUTPUT}"/otu_taxonomy_save.txt
...
sed -i .txt "/"$y"$/d" "${OUTPUT}"/otus_unknown.txt
done
rm "${OUTPUT}"/otus_unknown.txt.txt
